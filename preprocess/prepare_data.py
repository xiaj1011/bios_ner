...
def k_fold_samples(all_samples, mode):
    all_sample_len = len(all_samples)
    train_count, dev_count = -1, -1
    log.info("cfg.mode: %s" % mode)
    if mode == 'train':
    ...
    log.info("train_count {}, dev_count {}, test_count {} ".
             format(train_count, dev_count, all_sample_len - train_count - dev_count))
...
def generate(raw_text_path, predict_path, terms_pkl_path,
    ...
    train_samples, dev_samples, test_samples = k_fold_samples(all_samples, mode)
...
def generate_predict(raw_text_path, tagged_path, dataset_dir, mode):
    if not os.path.exists(tagged_path) or os.path.exists(raw_text_path):
        log.warning('{} or {} not exist'.format(tagged_path, raw_text_path))
        return

    all_samples = []

    original_reader = open(raw_text_path, 'r', encoding="utf-8")
    tagged_reader = open(tagged_path, 'r', encoding="utf-8")

    num_predict = sum([1 for i in open(tagged_path, "r")])
    for _, one_line in tqdm(enumerate(tagged_reader), total=num_predict):
        tagged_line = json.loads(one_line.rstrip())
        original_line = original_reader.readline().rstrip()

        entities = []
        for entity in tagged_line:
            if isinstance(entity, str):
                continue
            entities.append(entity)
