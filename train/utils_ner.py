...
class InputFeatures:
    ...
    input_ids: List[int]  # bert自带的token对应id
    attention_mask: List[int]
    token_type_ids: Optional[List[int]] = None
    label_ids: Optional[List[int]] = None  # labels.txt中的label对应的id，手动生成
        ...
        for (ex_index, example) in enumerate(tqdm(examples)):
            ...
            for word, label in zip(example.words, example.labels):
                if word in word_tokens_map:
                    word_tokens = word_tokens_map.get(word)
                else:
                    word_tokens = tokenizer.tokenize(word)
                    word_tokens_map.setdefault(word, word_tokens)
